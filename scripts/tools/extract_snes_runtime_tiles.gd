...
const _OUT_ROOT := "res://assets/terrain/runtime"
const _TILE_SIZE := 32
...
const _CATEGORY_SPECS := [
	...
	{
		"name": "transition_shallow_mid",
		"dir": _OUT_ROOT + "/water/transitions/shallow_to_mid",
		"prefix": "shallow_to_mid",
		"cells": [
	...
	{
		"name": "transition_mid_deep",
		"dir": _OUT_ROOT + "/water/transitions/mid_to_deep",
		"prefix": "mid_to_deep",
		"cells": [
...
func _initialize() -> void:
	...
	if err != OK:
		push_error("Failed to load SNES target reference sheet v2")
		quit(1)
		return
	...
	for spec_variant in _CATEGORY_SPECS:
		...
		for output_index in range(cells.size()):
			...
			tile.resize(_TILE_SIZE, _TILE_SIZE, Image.INTERPOLATE_BILINEAR)
			_seal_tile_edges(tile)
	...
	if file == null:
		push_error("Failed to write runtime tile manifest")
		quit(1)
		return
	file.store_string(JSON.stringify(manifest, "\t"))
	print("SNES runtime tiles exported to %s" % _OUT_ROOT)
	quit()


func _seal_tile_edges(tile: Image) -> void:
	...
	for y in range(h):
		var left_src := tile.get_pixel(2, y)
		var right_src := tile.get_pixel(w - 3, y)
	...
	for x in range(w):
		var top_src := tile.get_pixel(x, 2)
		var bottom_src := tile.get_pixel(x, h - 3)
